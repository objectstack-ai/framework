 ...
 * `sys_organization` (cloud-side). Without mirroring that org into the
 * project DB, the project would start empty and SecurityPlugin's
 * `ensureUserHasOrganization` middleware would auto-create a personal
 * "Alice's Workspace" for the project owner — completely disconnected
 * from the team workspace that owns the project at the platform level.
 *
 * Mirroring the owning org here makes the project's primary org match
 * the cloud team. Subsequent JIT-provisioned members of the same cloud
 * org get attached to this same row (Phase 2 — claim-driven by the SSO
 * callback).
 *
 * Idempotency
 * -----------
 ...
 * owner to the mirrored cloud org BEFORE SecurityPlugin's
 * `ensureUserHasOrganization` middleware would otherwise auto-create a
 * disjoint personal workspace.
 *
...
export async function seedProjectMember(
    ...
    try {
        ...
        // the existing convention in `ensureUserHasOrganization`.

...
- f169558: feat(org-scoping): hand a default org's seeded records to its admin (multi-tenant ownership handoff)
  ...
  Seeded rows land `owner_id` NULL (the author leaves it unset; `cel`os.user.id``resolves to NULL at seed time). In multi-tenant mode`claimOrphanOrgRows`back-fills their`organization_id`, but `owner_id` stayed NULL — so "My" views,

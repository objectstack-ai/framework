...
### Fixed
- **Build & test stability** — three issues blocking `pnpm build` / `pnpm test` from going green:
  - **`@objectstack/spec` — `SocialProviderConfigSchema.enabled` now actually defaults to `true`** as its `.describe()` text already promised. The field was declared `z.boolean().optional()` (no default), so `SocialProviderConfigSchema.parse({ google: { clientId, clientSecret } }).google.enabled` returned `undefined` instead of `true`, breaking the matching assertion in `packages/spec/src/system/auth-config.test.ts` and silently changing better-auth provider behaviour for callers that relied on the documented default. Changed to `z.boolean().default(true)` in `packages/spec/src/system/auth-config.zod.ts` so the runtime now matches the documented contract.
  - **`@objectstack/docs` build — replaced unsupported `env` Shiki language** in `content/docs/guides/auth-sso.mdx` (8 fenced blocks) with `bash`. Shiki's bundled grammar set does not include `env`, so Turbopack's MDX evaluation threw `ShikiError: Language 'env' not found` and aborted `next build`. `bash` is the convention used by ~96 other code blocks in `content/docs/` for shell / dotenv-style snippets.
  - **`@objectstack/plugin-dev` test — deflaked `should init with mocked context and handle missing deps gracefully`** in `packages/plugins/plugin-dev/src/dev-plugin.test.ts`. With default options, `DevPlugin.init()` performs ~10 dynamic `await import(...)` calls for real peer plugins (`@objectstack/objectql`, `@objectstack/runtime`, `@objectstack/driver-memory`, `@objectstack/plugin-auth`, `@objectstack/plugin-security`, `@objectstack/plugin-hono-server`, `@objectstack/rest`, `@objectstack/plugin-setup`, dispatcher, i18n). In this monorepo all of those resolve, so the test was implicitly exercising the full real-plugin init path — fast in isolation (~2 s) but racing the 15 s timeout under parallel `turbo run test` load. The test now disables every optional service in `DevPluginOptions.services`, which is what the test name has always advertised: drive the same try/catch graceful-skip branches that fire when peer packages are missing, deterministically and without depending on real-plugin init. Companion tests still cover the contract-stub path and the all-disabled path.

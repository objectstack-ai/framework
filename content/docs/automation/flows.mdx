...
designer surfaces the same data in its **Runs** side panel. Recent runs are held
in an in-memory ring buffer for full step detail, and every terminal run is also
mirrored to the durable `sys_automation_run` store (`completed` / `failed` rows,
kept under a per-flow cap and a 30-day retention sweep); `listRuns` merges the
two, so run history **survives a process restart** rather than starting fresh on
each boot. (Live `paused` runs are always durable there — that is what makes
resume work across a restart.)

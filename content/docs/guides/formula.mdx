...
type Expression = {
  dialect: 'cel' | 'js' | 'cron';
  source?: string;          // surface syntax (CEL text)
...
transforms it into `{ dialect: 'cel', source }`. The compiled artifact always
contains the full envelope (and, after M9.2, the AST).

```ts
import { defineObject, F, P, cel } from '@objectstack/spec';
  ...
  fields: [
    {
      name: 'total',
      type: 'formula',
      // tagged-template — produces { dialect: 'cel', source: '...' }
    ...
    },
    {
      name: 'po_number',
      type: 'text',
      // bare string is shorthand for { dialect: 'cel', source }
...
The three tagged-template helpers — `cel`, `F` (formula alias), `P`
(predicate alias) — are exported from `@objectstack/spec` and are
interchangeable; pick whichever reads best at the call site.

...
these as the baseline for any new tenant — tier custom sets on top rather
than redefining the basics.

| Name | Scope | What it unlocks | Notes |
|:--|:--|:--|:--|
| `admin_full_access` | Platform (no RLS) | All objects, all system permissions, Studio + Setup, cross-tenant reads | Reserved for platform operators. Auto-granted to the bootstrap admin (`bootstrapPlatformAdmin`). |

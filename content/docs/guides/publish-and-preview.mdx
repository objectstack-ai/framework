...
2. **Uploads** to object storage at `artifacts/${projectId}/${commitId}.json`
   (no-op if the key already exists — content-addressable).
...
Every publish is preserved. To roll back:

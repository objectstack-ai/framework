...
When the kernel runs in multi-tenant mode (`OS_MULTI_ORG_ENABLED=true`; the older
`OS_MULTI_TENANT` is still accepted as a deprecated alias), the registry also

...
| **body** | `Object \| Object` | optional | Action body — expression (L1) or sandboxed JS (L2). Only used when type is `script`. |
...
| **resultDialog** | `Object` | optional | Render API response in a one-shot reveal dialog (suppresses successMessage when set). |
| **visible** | `string \| Object` | optional | Visibility predicate (CEL). |
...
| **disabled** | `boolean \| string \| Object` | optional | Boolean or predicate (CEL) — action is disabled when TRUE. |
...
| **ai** | `Object` | optional | AI exposure (opt-in). Set ai.exposed=true + ai.description to make this callable by agents. |
...
| **bodyShape** | `string \| Object` | optional | Body wrapping: flat (default) or `{ wrap: key }` to nest user-collected params under a key. |
...
| **visible** | `string \| Object` | optional | Param visibility predicate (CEL); omits the param when false. |

...
> Rationale: [ADR-0021 — Analytics: one semantic dataset layer](https://github.com/objectstack-ai/objectstack/blob/main/docs/adr/0021-analytics-dataset-semantic-layer.md)
...
> Note: later releases moved the provider SDKs back to a host-install model — in
> the current CLI they are not bundled. The `ai` settings namespace states that
> provider SDK packages (e.g. `@ai-sdk/openai`) must be installed on the host for
> the chosen provider to load, and the AI runtime runs through the host-declared
> `@objectstack/service-ai` plugin, which auto-detects the provider from
> environment variables (`AI_GATEWAY_MODEL`, `OPENAI_API_KEY`, …).

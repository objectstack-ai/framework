...
- **`validateListViewMode`** (ADR-0047) — a `tabs`-style `userFilters`
  (`element: 'tabs'`) on an object list view is now a `tsc` error via
  `ObjectListViewSchema` (dropdown value chips remain allowed), stripped at
  parse for back-compat, and reported pre-parse with a fix hint.

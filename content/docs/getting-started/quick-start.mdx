...
The `os` commands below use the CLI that `init` installs into your project — if `os` isn't on your PATH, run them as `npx os …` (or `pnpm exec os …`).
...
Re-run the same command any time to update — the bundle is versioned as one unit. See the [AI Skills Reference](/docs/ai/skills-reference) for the full catalog.
...
<Cards>
  ...
  <Card
    ...
    href="/docs/concepts/design-principles"
...
- [Data Modeling Guide](/docs/data-modeling) &mdash; Deep dive into objects and fields

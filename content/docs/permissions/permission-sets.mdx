...
- **Editing a packaged (declared) set through Setup** becomes an environment
  overlay of that definition — it now genuinely takes effect, where a
  record-only edit previously displayed but never enforced.
...
An environment-scope customization of a **package-owned** set is refused at
authoring time (`package_owned`): edit it in its package and re-publish, or
clone it to a new environment-owned name.
